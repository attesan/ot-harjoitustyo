from project_point import ProjectPoint
...
class ProjectDevice:
    ...
    def __init__(self, device_type:str, device_points:list, device_position:str, point_name_separator:str = "_"):
        self.__type = device_type
        self.__points = []
        # Create point objects from database point data.
        for entry in device_points:
            self.__points.append(ProjectPoint(device_position, entry, "NotImplemented", point_name_separator))
    ...
    def __check_valid(self, data):
        ...
        for m in data:
            if m not in allowed or m == self.__separator:
                return False
        return True
